						...
						{#each teams as team}
							<div class="flex items-center justify-between">
								...
								<button
									...
									class="inline-flex items-center rounded-md bg-red-600 px-2.5 py-1.5 text-sm font-semibold text-white hover:bg-red-500"
								>
									Add to Overlay
								</button>
							</div>
					...
					{:else}
						<div class="space-y-4">
							{#each selectedTeams as { team, visible, position }}
								<div class="flex items-center justify-between rounded-md bg-gray-700 p-4">
									<div class="flex items-center">
										{#if team.logo_url}
											<img src={team.logo_url} alt="" class="h-8 w-8 flex-shrink-0 rounded-full" />
										{/if}
									...
									<div class="flex space-x-2">
										<button
											...
											class="inline-flex items-center rounded-md bg-gray-600 px-2.5 py-1.5 text-sm font-semibold text-white hover:bg-gray-500"
										>
											{visible ? 'Hide' : 'Show'}
										</button>
										<button
							...
							{#if selectedTeams.length === 2}
								<button
									...
									class="mt-4 w-full rounded-md bg-gray-600 px-3 py-2 text-sm font-semibold text-white hover:bg-gray-500"
								>
									Swap Positions
								</button>
							{/if}
							...
							<div class="mt-6">
								...
								<input
									...
									readonly
									value={getStreamUrl()}
									class="mt-1 w-full rounded-md bg-gray-700 px-3 py-2 text-sm text-white"
								/>
							</div>
						</div>
					{/if}
